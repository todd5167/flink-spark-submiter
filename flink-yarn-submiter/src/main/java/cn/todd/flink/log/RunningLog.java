...
public class RunningLog {
    ...
    public List<String> getRollingLogBaseInfo(JobParamsInfo jobParamsInfo, String applicatonId)
            throws Exception {
        ...
        String amRootURl =
                String.format(
                        APPLICATION_REST_API_TMP, UrlUtil.getHttpRootURL(rmUrl), applicatonId);
        ...
        try {
            ...
            if (!StringUtils.isEmpty(response)) {
                ...
                String amStatue =
                        applicationWSParser.getParamContent(ApplicationWSParser.AM_STATUE);

                if (!StringUtils.equalsIgnoreCase(amStatue, "RUNNING")) {
                    return result;
                }

                String amContainerLogsURL =
                        applicationWSParser.getParamContent(
                                ApplicationWSParser.AM_CONTAINER_LOGS_TAG);
                ...
                String trackingUrl =
                        applicationWSParser.getParamContent(ApplicationWSParser.TRACKING_URL);

                // parse am log
                parseAmLog(applicationWSParser, amContainerLogsURL).ifPresent(result::add);
                // parse containers log
                List<String> containersLogs =
                        parseContainersLog(
                                applicationWSParser, user, containerLogUrlFormat, trackingUrl);
    ...
    private String buildContainerLogUrl(
            String containerHostPortFormat, String[] nameAndHost, String user) {
        LOG.debug(
                "buildContainerLogUrl name:{},host{},user{} ",
                nameAndHost[0],
                nameAndHost[1],
                user);
     ...
     * @return
     */
    public Optional<String> parseAmLog(
            ApplicationWSParser applicationWSParser, String amContainerLogsURL) {
        try {
            String logPreURL = UrlUtil.getHttpRootURL(amContainerLogsURL);
            ApplicationWSParser.RollingBaseInfo amLogInfo =
                    applicationWSParser.parseContainerLogBaseInfo(amContainerLogsURL, logPreURL);
    ...
    private List<String> parseContainersLog(
            ApplicationWSParser applicationWSParser,
            String user,
        ...
        for (TaskmanagerInfo info : taskmanagerInfos) {
            ...
            ApplicationWSParser.RollingBaseInfo rollingBaseInfo =
                    applicationWSParser.parseContainerLogBaseInfo(
                            containerLogUrl, UrlUtil.getHttpRootURL(containerLogUrl));
    ...
    private List<TaskmanagerInfo> getContainersNameAndHost(String trackingUrl) throws IOException {
        ...
        try {
            ...
            containersNameAndHost =
                    IntStream.range(0, taskManagers.size())
                            .mapToObj(taskManagers::getJSONObject)
                            .map(
                                    jsonObject ->
                                            JSONObject.toJavaObject(
                                                    jsonObject, TaskmanagerInfo.class))
